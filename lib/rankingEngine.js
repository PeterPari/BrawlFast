 ...
 * @param {Array<Object>} teams - Team composition data
 * @param {string} mapMode - Map mode string
 * @param {Object} [stats] - Optional pre-calculated statistics
 * @returns {number} CPS score (typically 0.0 to 1.0, higher is better)
 */
function computeCPS(brawler, allBrawlers, teams, mapMode, stats = null) {
 ...
 *
 * This function optimizes performance by calculating global statistics once
 * and reuse them for each brawler's CPS calculation.
 *
 * @param {Array<Object>} brawlers - List of brawlers to rank
 * @param {Array<Object>} teams - Team composition data
 * @param {string} mapMode - Map mode string
 * @returns {Array<Object>} Ranked brawlers with CPS and tier
 */
function rankBrawlers(brawlers, teams, mapMode) {
  if (!brawlers || brawlers.length === 0) return [];

  // Pre-calculate distribution statistics
  ...
  const withCPS = brawlers.map(brawler => ({
    ...brawler,
    cps: computeCPS(brawler, brawlers, teams, mapMode, stats)
  }));
